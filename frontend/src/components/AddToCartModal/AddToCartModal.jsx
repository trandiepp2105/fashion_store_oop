...
const AddToCartModal = () => {
  const [isVisible, setIsVisible] = useState(true); // Trạng thái hiển thị modal

  useEffect(() => {
    const timer = setTimeout(() => {
      setIsVisible(false); // Sau 5 giây, ẩn modal
    }, 5000); // 5000ms = 5 giây

    // Dọn dẹp khi component bị unmount hoặc khi setTimeout hoàn thành
    return () => clearTimeout(timer);
  }, []);

  if (!isVisible) {
    return null; // Nếu không hiển thị, trả về null (không render gì cả)
  }
  ...
  return (
    <div className="add-to-cart-modal">
      <svg
        ...
        className="add-to-cart-modal__icon"
      >
      ...
      <h4 className="add-to-cart-modal__title">
        Sản phẩm đã được thêm và giỏ hàng
      </h4>
